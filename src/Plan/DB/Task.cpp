  ...
  namespace DB
  {
    ...
    struct Task : public DUNE::Tasks::Task
    {
      ...
      getPlanInfo(const IMC::PlanDB &req)
      {
        ...
        *m_query_plan_stmt >> m_plan_info.change_time >> m_plan_info.change_sid >> m_plan_info.change_sname >> m_plan_info.md5 >> m_plan_info.plan_size;
      ...
      getDatabaseState(const IMC::PlanDB &req)
      {
        ...
        while (m_plan_iterator_stmt->execute())
        {
          ...
          *m_plan_iterator_stmt >> pinfo->plan_id >> pinfo->change_time >> pinfo->change_sid >> pinfo->change_sname >> pinfo->md5 >> pinfo->plan_size;
        ...
        *m_lastchange_query_stmt >> state->change_time >> state->change_sid >> state->change_sname;
        m_lastchange_query_stmt->reset();
